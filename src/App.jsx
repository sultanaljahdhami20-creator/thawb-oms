...
export default function App(){
  ...
  const [newO,setNewO]=useState({customer:"",phone:"",jackets:"",total:"",paid:"",orderType:"",extras:"",deliveryPaid:true,deliveryArea:""});
  ...
  const deliveryText=(o)=>o.deliveryPaid===false?t.deliveryUnpaid:t.deliveryPaid;
  ...
  const openLabelPrint=(labelOrders)=>{
    ...
    const labels=printable.map(o=>{
      ...
      return `<section class="label" dir="rtl">
        ...
        <div class="delivery-payment"><strong>${o.deliveryPaid===false?"التوصيل غير مدفوع":"التوصيل مدفوع"}</strong><span>${o.deliveryPaid===false?"DELIVERY UNPAID":"DELIVERY PAID"}</span></div>
  ...
  const renderLabelCanvas=async(o,logo)=>{
    ...
    line(0,842,768,842);
    drawLabelText(ctx,o.deliveryPaid===false?"التوصيل غير مدفوع":"التوصيل مدفوع",384,877,28,{weight:900,align:"center",maxWidth:688});
    drawLabelText(ctx,o.deliveryPaid===false?"DELIVERY UNPAID":"DELIVERY PAID",384,903,18,{weight:800,align:"center",direction:"ltr",maxWidth:688});
      ...
      try{
        ...
        if(ords&&ords.length>0){
          setOrders(ords.map(o=>({
            ...
            deliveryPaid:o.delivery_paid!==false,
  ...
  const saveOrderEdit=async()=>{
    ...
    if(editOrderForm.deliveryPaid!==(o.deliveryPaid!==false)) changes.push(t.deliveryPayment+": "+deliveryText(o)+" → "+deliveryText(editOrderForm));
    ...
    setOrders(prev=>prev.map(x=>x.id!==o.id?x:{
      ...
      deliveryPaid:editOrderForm.deliveryPaid,
            ...
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                <thead><tr style={{background:dm?"#1A2744":C.slateLight}}>
                  ...
                  {[t.orderNum,t.customer,t.phone,t.date,t.jackets,t.extrasShort,t.total,t.paid,t.balance,t.status,t.supplier,t.deliveryArea,t.deliveryPayment,t.actions].map(h=><th key={h} style={{padding:"12px 14px",textAlign:"left",fontWeight:700,fontSize:11,textTransform:"uppercase",color:tm,whiteSpace:"nowrap"}}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {filtOrd.map((o,i)=>(
                    <tr key={o.id} style={{borderTop:"1px solid "+bc,background:selectedOrderIds.includes(o.id)?"#EEF2FF":o.isUrgent?"#FFF7ED":i%2===0?"transparent":"rgba(0,0,0,0.01)"}}>
                      ...
                      <td style={{padding:"12px 14px",fontSize:12,fontWeight:700,color:o.deliveryPaid===false?"#B45309":"#2D7A4F",whiteSpace:"nowrap"}}>{deliveryText(o)}</td>
          ...
          return <div>
            ...
            <div className="grid-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
              <div style={{background:bgC,border:"1px solid "+bc,borderRadius:12,padding:20}}>
                ...
                {[[t.name,o.customer||"--"],[t.phone,o.phone],[t.orderDate,o.date],[t.deliveryArea,o.deliveryArea||"--"],[t.deliveryPayment,deliveryText(o)]].map(([k,v])=>(
      ...
      {showNew&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&(setShowNew(false),setDupWarning(null))}>
        <div style={{background:bgC,borderRadius:16,padding:32,width:480,maxWidth:"95vw",maxHeight:"90vh",overflowY:"auto"}}>
          ...
          <div className="grid-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            ...
            <div><label htmlFor="newO-delivery-paid" style={{display:"block",fontSize:12,fontWeight:600,color:tm,marginBottom:5}}>{t.deliveryPayment}</label><select id="newO-delivery-paid" value={String(newO.deliveryPaid)} onChange={e=>setNewO(p=>({...p,deliveryPaid:e.target.value==="true"}))} style={IS}><option value="true">{t.deliveryPaid}</option><option value="false">{t.deliveryUnpaid}</option></select></div>
      ...
      {showEditOrder&&editOrderTarget&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&(setShowEditOrder(false),setEditOrderTarget(null))}>
        <div style={{background:bgC,borderRadius:16,padding:32,width:520,maxWidth:"95vw",maxHeight:"90vh",overflowY:"auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:8}}>
            ...
            <div><label htmlFor="editOrderForm-delivery-paid" style={{display:"block",fontSize:12,fontWeight:600,color:tm,marginBottom:5}}>{t.deliveryPayment}</label><select id="editOrderForm-delivery-paid" value={String(editOrderForm.deliveryPaid)} onChange={e=>setEditOrderForm(p=>({...p,deliveryPaid:e.target.value==="true"}))} style={IS}><option value="true">{t.deliveryPaid}</option><option value="false">{t.deliveryUnpaid}</option></select></div>
          ...
          <div style={{border:"2px solid "+bc,borderRadius:12,padding:20,marginBottom:20,background:"#fff",color:"#1E293B"}}>
            ...
            <div className="grid-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
              <div>
                ...
                {[[t.name,printO.customer||"--"],[t.phone,printO.phone],[t.date,printO.date],[t.deliveryPayment,deliveryText(printO)]].map(([k,v])=>(
